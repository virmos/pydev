...
def test_register_user(client):
    ...
    response = client.post(
        "/register", json={"username": username, "password": "Test Password"},
    )
...
def test_login_user_bad_password(client, created_user_details):
    ...
    response = client.post(
        "/login", json={"username": username, "password": "bad_password"},
    )
...
def test_login_user_bad_username(client, created_user_details):
    ...
    response = client.post(
        "/login", json={"username": "bad_username", "password": password},
    )
...
def test_logout_user(client, created_user_jwts):
    response = client.post(
        "/logout", headers={"Authorization": f"Bearer {created_user_jwts[0]}"},
    )
...
def test_logout_user_invalid_token(client):
    response = client.post("/logout", headers={"Authorization": "Bearer bad_token"},)
...
def test_get_user_details(client, created_user_details):
    response = client.get("/user/1",)  # assume user id is 1
...
def test_refresh_token_invalid(client):
    response = client.post("/refresh", headers={"Authorization": "Bearer bad_jwt"},)
...
def test_refresh_token(client, created_user_jwts):
    response = client.post(
        "/refresh", headers={"Authorization": f"Bearer {created_user_jwts[1]}"},
    )
