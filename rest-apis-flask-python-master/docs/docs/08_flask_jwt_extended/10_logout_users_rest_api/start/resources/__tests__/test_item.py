...
def test_delete_item(client, admin_jwt, created_item_id):
    response = client.delete(
        f"/item/{created_item_id}", headers={"Authorization": f"Bearer {admin_jwt}"},
    )
...
def test_delete_item_without_admin(client, jwt, created_item_id):
    response = client.delete(
        f"/item/{created_item_id}", headers={"Authorization": f"Bearer {jwt}"},
    )
...
def test_get_all_items(client, jwt):
    ...
    response = client.get("/item", headers={"Authorization": f"Bearer {jwt}"},)
...
def test_get_item_detail_not_found(client, jwt):
    response = client.get("/item/1", headers={"Authorization": f"Bearer {jwt}"},)
