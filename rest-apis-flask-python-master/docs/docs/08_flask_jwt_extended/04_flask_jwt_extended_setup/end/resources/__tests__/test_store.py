def test_get_store(client, created_store_id):
    response = client.get(f"/store/{created_store_id}",)
...
def test_get_store_with_item(client, created_store_id):
    ...
    assert response.status_code == 200
    assert response.json["items"] == [{"id": 1, "name": "Test Item", "price": 10.5,}]


def test_get_store_with_tag(client, created_store_id):
    client.post(
        f"/store/{created_store_id}/tag", json={"name": "Test Tag"},
    )
...
def test_create_store_with_items(client, created_store_id):
    client.post(
        "/item", json={"name": "Test Item", "price": 10.5, "store_id": 1},
    )
...
def test_delete_store(client, created_store_id):
    response = client.delete(f"/store/{created_store_id}",)
    ...
    assert response.json == [
        {
            "id": 1,
            "name": "Test Store",
            "items": [{"id": 1, "name": "Test Item", "price": 10.5,}],
            "tags": [],
        }
    ]
...
def test_get_store_list_with_tags(client):
    resp = client.post("/store", json={"name": "Test Store"},)
    client.post(
        f"/store/{resp.json['id']}/tag", json={"name": "Test Tag"},
    )
    response = client.get("/store",)
