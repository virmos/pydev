...
def test_delete_item(client, created_item_id):
    response = client.delete(f"/item/{created_item_id}",)
...
def test_update_item(client, created_item_id):
    response = client.put(
        f"/item/{created_item_id}", json={"name": "Test Item (updated)", "price": 12.5},
    )
...
def test_get_all_items(client):
    ...
    response = client.post(
        "/item", json={"name": "Test Item 2", "price": 10.5, "store_id": 1},
    )
